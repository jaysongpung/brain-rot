        ...
        <>
            ...
            <div className="logos" id="logo_blur_insta" data-sync-id="p1-logo-insta">
                ...
                <div className="logo_text">
                    인스타그램은 피드, 스토리, 릴스, 탐색, 검색 탭 등 모든 콘텐츠 영역에서 딥러닝 기반 추천 알고리즘이 사용됩니다. 인스타그램은 다양한 알고리즘과 분류 모델이 각각 목적을 갖고 있으며 각 영역에서 사람들이 사용하는 방식에 맞는 자체 알고리즘을 사용하고 있습니다.
                    특히 탐색 탭은 세계 수억 명의 사용자를 대상으로 한 대규모 추천 시스템입니다. 랭킹 퍼널이라 불리는 다단계 구조의 콘텐츠 추천이 이루어집니다. 랭킹 퍼널은 경량 모델로 먼저 추천 후보군을 추린 뒤, 정교한 딥러닝 모델로 추천 콘텐츠의 상호작용 확률을 예측합니다. 이때 사용자의 좋아요, 공유, 댓글, 무반응 등 다양한 반응은 물론 접속한 전자 기기가 무엇인지 까지 통합적으로 고려됩니다.
                </div>
            </div>
            <div className="d30"></div>
            <div className="d30"></div>
            <div className="logos" id="logo_blur_twitter" data-sync-id="p1-logo-twitter">
                ...
                <div className="logo_text">
                    X(구 트위터)의 추천 시스템은 크게 3단계로 구성됩니다. 첫 번째 단계는 추천할 게시물을 고르는 단계입니다. 수억 개의 트윗에서 사용자가 반응할 만한 약 1500개의 후보를 뽑습니다. 추천 게시물은 팔로우한 계정뿐만 아니라, 팔로우하지 않은 계정에서도 선별합니다. X는 이를 &apos;인 네트워크&apos;와 &apos;아웃 오브 네트워크&apos;가 명명합니다. 두 번째 단계는 약 1500개의 후보에 점수를 매기는 것입니다. 사용자가 추천에 반응할 확률을 예측하고 이를 종합해 순위를 매깁니다. 마지막으로 추천 게시물을 점검합니다. 사용자가 추천에 피로감을 느끼지 않게 앞서 부정적인 피드백이 있었던 게시물, 게시자를 걸러내죠. X는 &quot;추천 트윗을 만드는 과정은 하루에 약 50억 번 실행되며 모든 단계는 평균 1.5초 내로 이뤄진다&quot;
                    고 자사 블로그에서 설명합니다.
                </div>
            </div>
            <div className="d30"></div>
            ...
            <div className="logos" id="logo_blur_youtube" data-sync-id="p1-logo-youtube">
                ...
                <div className="logo_text">
                    2019년 구글이 발표한 논문에 따르면 유튜브의 추천 시스템은 여러 마리 토끼를 모두 잡는 것을 목표로 합니다. 사용자가 추천 콘텐츠를 &apos;클릭&apos;해 시청하는 것뿐만 아니라, 영상을 끝까지 보는 것, &apos;좋아요&apos;를 누르는 것, 추천 콘텐츠에 대한 만족도 조사에 높은 평점을 매기는 것까지가 유튜브의 추천 시스템 목표죠. 이를 위해 구글은 사용자의 반응을 각각 예측한 뒤, 이를 종합해 어떤 영상을 추천할지 결정하는 모델을 개발했습니다. 단순히 클릭을 유도하는 것을 넘어 사용자의 만족도까지 높이기 위함입니다.
                </div>
            </div>
            <div className="d30"></div>
            <div className="d30"></div>
